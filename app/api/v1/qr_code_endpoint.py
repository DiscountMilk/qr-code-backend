...
async def create_qr_code(project_id: int, user_id: str = Depends(get_current_user_id)) -> QRCodeResponse:
...
async def get_qr_codes(project_id: int, include_deleted: bool = False, user_id: str = Depends(get_current_user_id),) -> list[QRCodeResponse]:
...
async def get_qr_code(project_id: int, qrcode_id: int, user_id: str = Depends(get_current_user_id)) -> QRCodeResponse:
...
async def update_qr_code(project_id: int, qrcode_id: int, qr_code: QRCodeUpdate, user_id: str = Depends(get_current_user_id)) -> QRCodeResponse:
...
async def delete_qr_code(project_id: int, qrcode_id: int, user_id: str = Depends(get_current_user_id)):
